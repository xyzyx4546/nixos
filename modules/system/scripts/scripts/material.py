...
WALLPAPER_DIR = (
    f"{MATERIAL_DIR}/wallpapers/1440p"
    if os.getenv("HOST") == "desktop"
    else f"{MATERIAL_DIR}/wallpapers/1080p"
)
...
def setup(img):
    ...
    os.system(
        "ags -r \"(await import('file://$HOME/.config/ags/applyCss.js')).default()\""
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "IMAGE", help="Specify 'select', 'random', or provide an image file path."
    )
    ...
    if IMAGE == "random":
        while True:
            ...
            IMAGE_NEW = (
                ...
                .resize(IMAGE_OLD.size)
                .convert(IMAGE_OLD.mode)
            )
        ...
        IMAGE = (
            os.popen(f"zenity --file-selection --filename {WALLPAPER_DIR}/*")
            .read()
            .strip()
        )
